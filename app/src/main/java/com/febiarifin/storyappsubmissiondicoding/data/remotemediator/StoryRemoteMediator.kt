...
import com.febiarifin.storyappsubmissiondicoding.database.RemoteKeys
...
class StoryRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            ...
            val dataStory = responseData.listStories

            val endOfPaginationReached = dataStory.isEmpty()
            ...
            database.withTransaction {
                ...
                val keys = dataStory.map {
                    RemoteKeys(id = it.id, prevKey = prevKey, nextKey = nextKey)
                }
                database.remoteKeysDao().insertAll(keys)
                storyDao.insertStory(dataStory)
            }
